...
function ChatItem({ chatId, room, userList, myId, isPinned, onTogglePin }: ChatItemProps) {
  ...
  const ChatPreviewImage = (chatId: string, memberIds: string[], userList: Types.UserList) => {
    if (memberIds.length === 0) {
      return <></>;
    } else if (memberIds.length === 1) {
      return (
        <ProfileImage
          key={`${chatId}-profile-0`}
          className={`w-[48px] h-[48px] ${userList[memberIds[0]].profileColor}`}
        />
      );
    } else {
      ...
      return (
        <div className="relative w-[48px] h-[48px] gap-[4px] flex flex-wrap justify-center items-center">
          {colorItems.map((color, idx) => (
            <ProfileImage key={`${chatId}-profile-${idx}`} className={`w-[22px] h-[22px] ${color}`} />
          ))}
        </div>
      );
    }
